...
function validateMovie(data) {
  ...
  if (type === "film") {
    if (!year || year < 1900 || year > currentYear)
      errors.push(`Год фильма от 1900 до ${currentYear}`);
  } else {
    if (!year_start || year_start < 1900 || year_start > currentYear)
      errors.push("Год начала сериала от 1900 до текущего");
...
router.post("/", (req, res) => {
  ...
  if (type === "film") {
    db.get(
      'SELECT id FROM movies WHERE title = ? AND type = "film" AND year = ?',
      [title, year],
      (err, row) => {
        if (row)
          return res
            .status(400)
            .json({ success: false, error: "Фильм с таким названием и годом уже есть" });
        insert();
      }
    );
  } else {
    ...
    db.get(sql, [title, year_start, year_end, year_end], (err, row) => {
      if (row)
        return res
          .status(400)
          .json({ success: false, error: "Сериал с таким названием и годами уже есть" });
      insert();
    });
  }
});
